...
public class FlipCDCManager extends CDCManager {
  ...
  private final Map<ByteBuffer, Integer> frontFeatureToLength = new HashMap<>(2048);
  private final Map<ByteBuffer, Integer> endFeatureToLength = new HashMap<>(2048);
  public long count = 0;

  public FlipCDCManager(String filePath) {
  ...
  public FlipCDCManager() {}

  private int rollHash(int preHash, byte nextByte) {
    return (preHash << 1) | (nextByte & 0x01);
  }
    ...
    try (BufferedInputStream bis =
      ...
      while ((bytesReadFromStream =
        ...
        while (currentParsePos < currentDataLimitInBuffer) {
          ...
          long currentDynamicMaskToUse = MASK_BITS_TARGET_LARGER_CHUNK;

          int rollingHashForCDC = 0;
          int cdcCutPointInBuffer = -1;
          int bytesScannedByCDC = 0;

          for (int scanIdx = cdcScanEffectiveStartOffset;
              scanIdx < currentDataLimitInBuffer;
              scanIdx++) {
            rollingHashForCDC = rollHash(rollingHashForCDC, buffer[scanIdx]);
            bytesScannedByCDC++;
            int currentTotalChunkCandidateSize = initialSkippedLength + bytesScannedByCDC;

            boolean cutConditionMetByCDC = false;
            if (currentTotalChunkCandidateSize >= MAX_SIZE) {
              cutConditionMetByCDC = true;
            } else if (currentTotalChunkCandidateSize >= MIN_SIZE) {
              if (currentTotalChunkCandidateSize >= NORMAL_CHUNK_SIZE_FOR_MASK_DECISION)
                currentDynamicMaskToUse = MASK_BITS_TARGET_SMALLER_CHUNK;
  ...
  public static void main(String[] args) {
    ...
    System.out.println("=====================================================");
    System.out.println("          FlipCDC algorithm");
    System.out.println("=====================================================");
    ...
    FlipCDCManager originalManager = new FlipCDCManager(filePathOriginal);
    ...
    FlipCDCManager updatedManager = new FlipCDCManager(filePathUpdate);
